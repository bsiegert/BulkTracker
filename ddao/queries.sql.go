...
const putFailedDep = `-- name: PutFailedDep :exec
INSERT INTO failed_deps
(from_result, on_result)
VALUES (?, ?)
`

type PutFailedDepParams struct {
	FromResult int64
	OnResult   int64
}

func (q *Queries) PutFailedDep(ctx context.Context, arg PutFailedDepParams) error {
	_, err := q.db.ExecContext(ctx, putFailedDep, arg.FromResult, arg.OnResult)
	return err
}
...
const putResult = `-- name: PutResult :one
INSERT INTO results
...
func (q *Queries) PutResult(ctx context.Context, arg PutResultParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, putResult,
		arg.BuildID,
		arg.PkgID,
		arg.PkgName,
		arg.BuildStatus,
